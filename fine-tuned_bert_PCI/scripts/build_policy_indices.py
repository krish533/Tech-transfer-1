...
def main():
    ...
    required = {"Institution", "Year", "Sentence_Cleaned", "P_restrictive_calib", "P_neutral_calib", "P_supportive_calib"}
    ...
    grouped = df.groupby(["Institution", "Year"], dropna=False)
    panel = grouped.agg(
    ...
    panel[keep_cols].sort_values(["Institution", "Year"]).to_csv(out_path, index=False)
