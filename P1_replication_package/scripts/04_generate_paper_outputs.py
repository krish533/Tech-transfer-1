...
def build_ranking_tex(ranked: pd.DataFrame, appendix_dir: Path) -> None:
    ...
    lines = [
        ...
        "",
        r"\begin{longtable}{@{}p{3.55cm}ccccccccccc@{}}",
...
def main() -> None:
    ...
    df = pd.read_csv(data_dir / "policy_level_indices_institution_year.csv")
    meta = pd.read_csv(data_dir / "institution_metadata.csv")
    df = df.merge(meta, on="Institution", how="left", validate="many_to_one")
