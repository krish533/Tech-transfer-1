...
def main() -> None:
    ...
    subprocess.run([sys.executable, str(scripts_dir / "01_build_input_corpus.py")], check=True)
    subprocess.run(
        [
            sys.executable,
            str(scripts_dir / "02_score_sentences.py"),
            "--model-dir",
            str(model_dir),
            "--temperature-file",
    ...
    subprocess.run(
        [
            sys.executable,
            str(scripts_dir / "03_build_policy_level_indices.py"),
            "--rerun-file",
            str(intermediate_dir / "policy_sentences_scored_raw_rerun.csv"),
            "--out-dir",
            str(derived_dir),
