...
def resolve_temperature(args, model, tokenizer, device: str) -> float:
    if args.temperature is not None:
        return float(args.temperature)

    if args.temperature_file:
        data = json.loads(Path(args.temperature_file).read_text(encoding="utf-8"))
        return float(data["temperature"])

    if args.calib_file:
        calibration_df = read_table(Path(args.calib_file))
        return learn_temperature(
            model=model,
            tokenizer=tokenizer,
    ...
    raise ValueError("Provide either --temperature, --temperature-file, or --calib-file.")
...
def main():
    ...
    parser.add_argument("--calib-text-col", default="Sentence")
    parser.add_argument("--calib-label-col", default="Label_Text")
    ...
    temperature = resolve_temperature(args, model, tokenizer, device)
